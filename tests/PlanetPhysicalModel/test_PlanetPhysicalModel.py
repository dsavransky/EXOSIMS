    ...
    def test_calc_albedo_from_sma(self):
        ...
        for mod in self.allmods:
            if 'calc_albedo_from_sma' in mod.__dict__:
                ...
                self.assertTrue(np.all(p >=0.0),'negative albedo value returned for %s'%mod.__name__)
    ...
    def test_calc_radius_from_mass(self):
        ...
        for mod in self.allmods:
            if 'calc_radius_from_mass' in mod.__dict__:
                ...
                self.assertTrue(len(Rp) == len(Mp),'length of radius array does not match input mass array for %s'%mod.__name__)
                self.assertTrue(Rp.unit is u.earthRad,'radius unit is not earthRad for %s'%mod.__name__)
                self.assertTrue(np.all(np.isfinite(Rp)),'Infinite radius value returned for %s'%mod.__name__)
                self.assertTrue(np.all(Rp > 0.0),'negative radius value returned for %s'%mod.__name__)
    ...
    def test_calc_mass_from_radius(self):
        ...
        for mod in self.allmods:
            if 'calc_mass_from_radius' in mod.__dict__:
                ...
                self.assertTrue(len(Mp) == len(Rp),'length of mass array does not match input radius array for %s'%mod.__name__)
                self.assertTrue(Mp.unit is u.earthMass,'mass unit is not earthMass for %s'%mod.__name__)
                self.assertTrue(np.all(np.isfinite(Mp)),'Infinite mass value returned for %s'%mod.__name__)
                self.assertTrue(np.all(Mp > 0.0),'negative mass value returned for %s'%mod.__name__)
