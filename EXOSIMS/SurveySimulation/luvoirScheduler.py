...
class luvoirScheduler(SurveySimulation):
    ...
    def observation_characterization(self, sInd, mode, mode_index):
        ...
        Args:
            ...
            mode_index (int):
                The index of the current spectral mode
        ...
        det = self.lastDetected[sInd,0,mode_index]
        ...
        if np.any(tochar):
            ...
            fEZ = self.lastDetected[sInd,1,mode_index][det][tochar]/u.arcsec**2
            dMag = self.lastDetected[sInd,2,mode_index][det][tochar]
            WA = self.lastDetected[sInd,3,mode_index][det][tochar]*u.arcsec
        ...
        if np.any(tochar):
            ...
            SNRfa = []
            if pIndsChar[-1] == -1:
                fEZ = self.lastDetected[sInd,1,mode_index][-1]/u.arcsec**2
                dMag = self.lastDetected[sInd,2,mode_index][-1]
                WA = self.lastDetected[sInd,3,mode_index][-1]*u.arcsec
            ...
            WAchar = self.lastDetected[sInd,3,mode_index][char]*u.arcsec
            # find the current WAs of characterized planets
            WAs = systemParams['WA']
            if FA:
                WAs = np.append(WAs, self.lastDetected[sInd,3,mode_index][-1]*u.arcsec)
