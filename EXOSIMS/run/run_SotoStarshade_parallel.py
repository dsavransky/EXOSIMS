...
def get_dMMap(TL,sInd,angles,tA,dtRange,m0,filename):
    ...
    for i,t in enumerate(dtFlipped):
        s_coll, t_coll, e_coll, TmaxRange = ens.collocate_Trajectory_minEnergy(TL,0,sInd,tA,t,m0)
        ...
        A = {'dMmap':dMmap,'eMap':eMap,'angles':angles,'dtRange':dtRange,'time':toc-tic,\
             'startTime':process_StartTime,'currentTime':process_CurrentTime,'mass':ens.mass,\
...
if __name__ == "__main__":
    ...
    tA = Time(np.array(60634., ndmin=1, dtype=float),format='mjd', scale='tai')
