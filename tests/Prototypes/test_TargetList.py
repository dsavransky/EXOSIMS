...
class Test_TargetList_prototype(unittest.TestCase):
    ...
    def test_outside_IWA_filter(self):
        ...
        self.targetlist.dist[10] = (
            21 * u.pc
        )  # rrange is 1e-3 to 200au, so this planet is below the IWA of 10 arcsec
    ...
    def test_main_sequence_filter(self):
        ...
        self.assertEqual(
            np.any(
                (self.targetlist.BV < 0.74)
                & (self.targetlist.MV > 6 * self.targetlist.BV + 1.8)
            ),
            False,
        )
        self.assertEqual(
            np.any(
                (self.targetlist.BV >= 0.74)
                & (self.targetlist.BV < 1.37)
                & (self.targetlist.MV > 4.3 * self.targetlist.BV + 3.05)
            ),
            False,
        )
        self.assertEqual(
            np.any(
                (self.targetlist.BV >= 1.37)
                & (self.targetlist.MV > 18 * self.targetlist.BV - 15.7)
            ),
            False,
        )
        self.assertEqual(
            np.any(
                (self.targetlist.BV < 0.87)
                & (self.targetlist.MV < -8 * (self.targetlist.BV - 1.35) ** 2 + 7.01)
            ),
            False,
        )
        self.assertEqual(
            np.any(
                (self.targetlist.BV >= 0.87)
                & (self.targetlist.BV < 1.45)
                & (self.targetlist.MV > 5 * self.targetlist.BV + 0.81)
            ),
            False,
        )
        self.assertEqual(
            np.any(
                (self.targetlist.BV >= 1.45)
                & (self.targetlist.MV < 18 * self.targetlist.BV - 18.04)
            ),
            False,
        )
    ...
    def test_fgk_filter(self):
        ...
        for i in range(len(self.targetlist.Spec)):
            ...
            assert (
                self.targetlist.Spec[i][0] == "F"
                or self.targetlist.Spec[i][0] == "G"
                or self.targetlist.Spec[i][0] == "K"
            )
    ...
    def test_completeness_specs(self):
        ...
        self.assertNotEqual(
            tl.PlanetPopulation.__class__.__name__,
            tl.Completeness.PlanetPopulation.__class__.__name__,
        )
    ...
    def test_starprop(self):
        ...
        t_refEqual = Time(
            timeRangeEql, format="jyear"
        )  # 1x5 time array, equal to sInds size
        t_refCopy = Time(
            np.tile(timeRange[0], 5), format="jyear"
