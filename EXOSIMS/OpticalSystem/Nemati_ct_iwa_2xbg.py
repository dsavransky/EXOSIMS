...
#        main_gs = GridSpec(1, 2, width_ratios=[1, 0.05], wspace=0.1)
#        left_gs = main_gs[0].subgridspec(1, 2, width_ratios=[1, 0.2], wspace=0)
#        ax_scatter = fig.add_subplot(left_gs[0])
#        scatter = ax_scatter.scatter(dMags, perc_errs, c=TL.Vmag, s=4, cmap="viridis")
#        ax_scatter.set_xlabel(r"$\Delta$mag$_0$")
#        ax_scatter.set_ylabel(r"Analytic $\Delta$mag$_0$ Percent Error (%)")
#        ax_histy = fig.add_subplot(left_gs[1], sharey=ax_scatter)
#        ax_histy.hist(perc_errs, bins=100, orientation="horizontal", color="k")
#        ax_histy.set_xlabel("Star Count")
#        ax_histy.set_ylabel("")
#        ax_histy.yaxis.set_visible(False)
#        ax_histy.set_ylim(ax_scatter.get_ylim())
#        cbar_ax = fig.add_subplot(main_gs[1])
#        cbar = fig.colorbar(scatter, cax=cbar_ax, label="V mag")
#        fig.suptitle(r"Analytic $\Delta$mag$_0$ Errors", fontsize=14)
#        plt.tight_layout(rect=[0, 0, 1, 0.95])
#        plt.savefig(f"Error_hist_{mode['instName']}.png", dpi=300)
        return dMags
    ...
    def get_coro_param(
        ...
        # now the case where we just got a scalar input
            ...
            if param_name == "core_area":
                ...
                #print(D*(7.87/500e-9*np.pi/180/3600)**2)
