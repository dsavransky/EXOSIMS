# -*- coding: utf-8 -*-
from EXOSIMS.SurveySimulation.linearJScheduler_DDPC_sotoSS import (
...
class linearJScheduler_3DDPC_sotoSS(linearJScheduler_DDPC_sotoSS):
    """linearJScheduler_3DDPC_sotoSS - linearJScheduler 3 Dual Detection
    Parallel Characterization SotoStarshade
    ...
    def run_sim(self):
        ...
        while not TK.mission_is_over(OS, Obs, det_modes[0]):
            ...
            if sInd is not None:
                ...
                if OS.haveOcculter:
                    # advance to start of observation_detection
                ...
                cmodes = [
                    cm
                    for cm in char_modes
                    if cm["systName"][-2] == det_mode["systName"][-2]
                ]
    ...
    def next_target(self, old_sInd, modes):
        ...
        for mode in modes:
            ...
            if len(mode_sInds.tolist()) > 0:
                if OS.haveOcculter and old_sInd is not None:
                    (
                        ...
                        old_sInd, mode_sInds, slewTimes, obsTimes, sd, mode
                    )
            ...
            for t in mode_sInds:
                if (intTimes[t] < all_intTimes[t] and intTimes[t] > 0) or all_intTimes[
                    t
                ] == 0:
        ...
        if len(sInds) > 0:
            ...
            s_IWA_OWA = (
                PP.arange * np.sqrt(TL.L[sInd]) / TL.dist[sInd]
            ).value * u.arcsec
            for bmode in blue_modes:
                intTime = self.calc_targ_intTime(sInd, startTimes[sInd], bmode)[0]
                if intTime != 0.0 * u.d:
                    if (
                        s_IWA_OWA[0] < bmode["IWA"] < s_IWA_OWA[1]
                        or s_IWA_OWA[0] < bmode["OWA"] < s_IWA_OWA[1]
                    ):
                        b_overlap = max(
                            0,
                            min(s_IWA_OWA[1], bmode["OWA"])
                            - max(s_IWA_OWA[0], bmode["IWA"]),
                        )
                        d_overlap = max(
                            0,
                            min(s_IWA_OWA[1], det_mode["OWA"])
                            - max(s_IWA_OWA[0], det_mode["IWA"]),
                        )
                        if b_overlap > d_overlap:
                            det_mode = copy.deepcopy(bmode)
                        elif b_overlap == d_overlap:
                            if (bmode["OWA"] - bmode["IWA"]) > (
                                det_mode["OWA"] - det_mode["IWA"]
                            ):
                                det_mode = copy.deepcopy(bmode)
            r_mode = [
                mode
                for mode in modes
                if mode["systName"][-1] == "r"
                and mode["systName"][-2] == det_mode["systName"][-2]
            ][0]
            if self.int_WA[sInd] > r_mode["IWA"] and self.int_WA[sInd] < r_mode["OWA"]:
                det_mode["BW"] = det_mode["BW"] + r_mode["BW"]
                det_mode["OWA"] = r_mode["OWA"]
                det_mode["inst"]["sread"] = (
                    det_mode["inst"]["sread"] + r_mode["inst"]["sread"]
                )
                det_mode["inst"]["idark"] = (
                    det_mode["inst"]["idark"] + r_mode["inst"]["idark"]
                )
                det_mode["inst"]["CIC"] = (
                    det_mode["inst"]["CIC"] + r_mode["inst"]["CIC"]
                )
                det_mode["syst"]["optics"] = np.mean(
                    (det_mode["syst"]["optics"], r_mode["syst"]["optics"])
                )
        ...
        # if no observable target, advanceTime to next Observable Target
            ...
            self.vprint(
                "No Observable Targets at currentTimeNorm = {}".format(
