...
class SurveySimulation(object):
    ...
    def __init__(self, **specs):
        ...
        Input: specs - dictionary containing user specification values and 
        desired module names"""
        
        # get desired module names (prototype or specific)
        # initialize names to prototype names
        simuniname = 'SimulatedUniverse'
        obsname = 'Observatory'
        timename = 'TimeKeeping'
        ppname = 'PostProcessing'
        
        names = ['simuniname', 'obsname', 'timename', 'ppname']
        # rewrite if named in specs
        for name in names:
            if name in specs:
                if name == 'simuniname':
                    simuniname = specs[name]
                elif name == 'obsname':
                    obsname = specs[name]
                elif name == 'timename':
                    timename = specs[name]
        ...
        SimUni = get_module(simuniname, 'SimulatedUniverse')
        # import observatory class
        Obs = get_module(obsname, 'Observatory')
        # import timekeeping class
        TK = get_module(timename, 'TimeKeeping')
        # import postprocessing class
        PP = get_module(ppname, 'PostProcessing')
        
        self.simuni = SimUni(**specs)
        self.obs = Obs(**specs)
        self.timekeeping = TK(**specs)
        self.post = PP(**specs)
        
        # bring inherited class objects to top level of Survey Simulation
        self.opt = self.simuni.opt # optical system class object
        self.pop = self.simuni.pop # planet population class object
        self.zodi = self.simuni.zodi # zodiacal light class object
        self.comp = self.simuni.comp # completeness class object
        self.planphys = self.simuni.planphys # planet physical model class object
        self.targlist = self.simuni.targlist # target list class object
    ...
    def run_sim(self):
        ...
        visited = np.zeros((len(self.targlist.dist),), dtype=int)
        # target revisit list
        revisit_list = np.array([])
        # current time (normalized to zero at mission start) of planet positions
        planPosTime = np.array([self.timekeeping.currenttimeNorm.to(u.day).value]*self.simuni.nPlans)*u.day
        # number of planet observations
        observed = np.zeros((self.simuni.nPlans,), dtype=int)
        # set occulter separation if haveOcculter
        if self.opt.haveOcculter:
            self.obs.currentSep = self.obs.occulterSep
        
        extended_list = np.array([])
        
        # initialize run options
        # perform characterization if doChar = True
        if self.opt.SNchar > 0.:
            doChar = True
        else:
            doChar = False
        
        # keep track of spectral characterizations, 0 is no characterization           
        spectra = np.zeros((self.simuni.nPlans,1), dtype=int)
        ...
        while self.timekeeping.currenttimeNorm < self.timekeeping.missionFinishNorm:
            ...
            if self.opt.haveOcculter:
                DRM['sc_mass'] = self.obs.sc_mass.to(u.kg).value
            ...
            pInds = np.where(self.simuni.planInds == s_ind)[0]
            ...
            FA, DET, MD, NULL = self.post.det_occur(observationPossible)
            ...
            if pInds.shape[0] != 0:
                if DET or FA:
                    ...
                    if np.isscalar(s.value):
                        mu = const.G*(self.simuni.Mp[pInds] + self.targlist.MsTrue[s_ind]*const.M_sun)
                    else:
                        muind = np.where(np.min(s))[0][0]
                        mu = const.G*(self.simuni.Mp[pInds[muind]] + self.targlist.MsTrue[s_ind]*const.M_sun)    
                    ...
                    T = 2.*np.pi*np.sqrt(self.pop.arange.mean()**3/mu)
                    t_rev = self.timekeeping.currenttimeNorm + 0.75*T
            else:
                # revisit based on average of population semi-major axis and mass
                Mp = self.simuni.Mp.mean()
                mu = const.G*(Mp + self.targlist.MsTrue[s_ind]*const.M_sun)
                T = 2.*np.pi*np.sqrt(self.pop.arange.mean()**3/mu)
                t_rev = self.timekeeping.currenttimeNorm + 0.75*T
            ...
            self.targlist.comp0 = self.comp.completeness_update(s_ind, self.targlist, obsbegin, obsend, nexttime)
            
            # acquire a new target star index
            s_ind, DRM = self.next_target(s_ind, self.targlist, revisit_list, extended_list, DRM)
            ...
            if self.opt.haveOcculter:
                if self.obs.sc_mass < self.obs.dryMass:
...
#                self.timekeeping.currenttimeNorm = 100*u.year                       
    ...
    def initial_target(self):
        ...
        dt = 1.*u.day
        while sinds.size == 0:
            a = self.obs.keepout(self.timekeeping.currenttimeAbs, self.targlist, self.opt.telescopeKeepout)
            # find observable targets at current mission time            
            sinds = np.where(self.obs.kogood)[0]
        ...
        s0 = self.targlist.comp0[sinds].argmax()
        s_ind = sinds[s0]
    ...
    def observation_detection(self, pInds, s_ind, DRM, planPosTime):
        ...
        dMag = self.opt.dMagLim - np.abs(np.random.randn(1)*0.5)
        Ip = (9.5e7/(u.m**2)/u.nm/u.s)*10.**(-(self.targlist.Vmag[s_ind]+dMag)/2.5) 
        # apparent separation placeholders
        if pInds.size == 0:
            s = np.array([1.])*u.AU
        else:
            s = np.sqrt(self.simuni.r[pInds][:,0]**2+self.simuni.r[pInds][:,1]**2)
        ...
        if pInds.shape[0] != 0:
            ...
            if planPosTime[pInds][0] != self.timekeeping.currenttimeNorm:
                ...
                try:
                    self.simuni.r[pInds], self.simuni.v[pInds] = \
                    self.simuni.prop_system(self.simuni.r[pInds], 
                                        self.simuni.v[pInds], 
                                        self.simuni.Mp[pInds], 
                                        self.targlist.MsTrue[self.simuni.planInds[pInds]], 
        ...
        if np.any(observationPossible):
            observationPossible, s = self.check_IWA_OWA(observationPossible,self.simuni.r[pInds],self.targlist.dist[s_ind],self.opt.IWA,self.opt.OWA)

        # determine if planets are bright enough
        if np.any(observationPossible):
            observationPossible, dMag = self.check_brightness(observationPossible, self.simuni.r[pInds], self.simuni.Rp[pInds], self.simuni.p[pInds], self.opt.dMagLim)

        # set integration time to max integration time as a default
        t_int = self.targlist.maxintTime[s_ind]
        ...
        if np.any(observationPossible):
            ...
            t_trueint = self.opt.calc_intTime(self.targlist, self.simuni, s_ind, pInds)
            # update observationPossible
            observationPossible = np.logical_and(observationPossible, t_trueint <= self.opt.intCutoff) 
    ...
    def observation_characterization(self, observationPossible, pInds, s_ind, spectra, s, Ip, DRM, FA, t_int):
        ...
        if pInds.shape[0] != 0:
            if np.any(spectra[pInds[observationPossible],0] == 0):
                ...
                throughput = self.opt.throughput(self.opt.specLambda, np.arctan(s/(self.targlist.dist[s_ind]*u.pc)))
                contrast = self.opt.contrast(self.opt.specLambda, np.arctan(s/(self.targlist.dist[s_ind]*u.pc)))
                ...
                charPossible = np.logical_and(charPossible, t_char <= self.opt.intCutoff)
                chargo = False
                ...
                if chargo:
                    ...
                    if self.opt.haveOcculter:
                        ...
                        deltaV_char = dF_lateral/self.obs.sc_mass*t_char
                        self.obs.sc_mass -= mass_used_char
                    ...
                    if np.any(charPossible):
                        if self.opt.haveOcculter:
                            ...
                            nld = self.opt.IWA.to(u.rad).value*np.sqrt(
                            self.opt.pupilArea/self.opt.shapeFac)/self.opt.lam
                            lambdaeff = (np.arctan(s/(self.targlist.dist[s_ind]*u.pc)).to(u.rad).value)
                            lambdaeff = lambdaeff*np.sqrt(self.opt.pupilArea/self.opt.shapeFac)
    ...
    def check_visible_end(self, observationPossible, t_int, t_trueint, s_ind, pInds, t_char_calc):
        ...
        for i in xrange(len(observationPossible)):
            if observationPossible[i]:
                r = self.simuni.r[pInds[i]]
                v = self.simuni.v[pInds[i]]
                Mp = self.simuni.Mp[pInds[i]]
                MsTrue = self.targlist.MsTrue[s_ind]
                Rp = self.simuni.Rp[pInds[i]]
                p = self.simuni.p[pInds[i]]
                if t_char_calc:
                    dt = t_int[i]
                else:
                    dt = t_trueint[i] + self.obs.settling_time
                ...
                if obsRes == 1:
                    ...
                    if not t_char_calc:
                        ...
                        if t_int == self.targlist.maxintTime[s_ind]:
                            t_int = t_trueint[i]
                        else:
    ...
    def sys_end_res(self, dt, s_ind, r, v, Mp, MsTrue, Rp, p):
        ...
        a = self.obs.keepout(self.timekeeping.currenttimeAbs+dt, self.targlist, self.opt.telescopeKeepout)
        # if star is not inside keepout zone at end of observation, see if 
        # planet is still observable
        if self.obs.kogood[s_ind]:
            # propagate planet to observational period end and find dMag
            rend, vend = self.simuni.prop_system(r, v, Mp, MsTrue, dt)
            # calculate dMagend
            ...
            if dMagend <= self.opt.dMagLim and send >= self.targlist.dist[s_ind]*u.pc*np.tan(self.opt.IWA):
                obsRes = 1
            else:
                obsRes = 0
        else:
    ...
    def det_data(self, s, dMag, Ip, DRM, FA, DET, MD, s_ind, pInds, observationPossible, observed):
        ...
        if FA:
            ...
            s = self.targlist.dist[s_ind]*u.pc*np.tan(self.opt.IWA)
            if self.pop.scaleOrbits:
                s += np.random.rand()*(self.pop.arange.max() - self.pop.arange.min())*np.sqrt(self.targlist.L[s_ind])
            else:
                s += np.random.rand()*(self.pop.arange.max() - self.pop.arange.min())
            dMag = self.opt.dMagLim - np.abs(np.random.randn()*0.5)
            Ip = (9.5e7/(u.m**2)/u.nm/u.s)*10.**(-(self.targlist.Vmag[s_ind]+dMag)/2.5)
        elif MD:
            ...
            if pInds.shape[0] == 1:
                ...
                DRM['det_WA'] = np.arctan(s/(self.targlist.dist[s_ind]*u.pc)).to(u.marcsec)[0].value
                DRM['det_dMag'] = dMag[0]
            else:
                DRM['det_status'] = observationPossible.astype(int).tolist()
                DRM['det_WA'] = np.arctan(s/(self.targlist.dist[s_ind]*u.pc)).to(u.marcsec).min().value
    ...
    def find_t_char(self, throughput, contrast, Ip, FA, s_ind, pInds):
        ...
        QE = self.opt.QE(self.opt.specLambda)
        FluxB = 9.5e7/(u.m**2)/u.nm/u.s
        
        # planet count
        cp = QE*self.opt.eta2*(self.opt.specLambda/self.opt.Rspec)*(Ip/(9.5e7/
        (u.m**2)/u.nm/u.s))*FluxB*throughput*self.opt.pupilArea
        # star count
        cs = QE*self.opt.eta2*(self.opt.specLambda/self.opt.Rspec)*(
        self.opt.pupilArea)*FluxB*10.**(-self.targlist.Vmag[s_ind]/2.5)*contrast/2
        
        # zodi count
        if FA:
            temp = self.zodi.fzodi(s_ind, 0., self.targlist)
            Z = FluxB*10.**(-23.54/2.5)*(1/u.arcsec**2)*temp*(self.opt.specLambda/
            self.opt.Rspec)*self.opt.pupilArea*(self.opt.pixelArea/
            (self.opt.focalLength**2))*u.sr
        else:
            Z = FluxB*10.**(-23.54/2.5)*(1/u.arcsec**2)*(
            self.simuni.fzodicurr[pInds])*(self.opt.specLambda/self.opt.Rspec)*(
            self.opt.pupilArea)*(self.opt.pixelArea/(self.opt.focalLength**2))*u.sr
        
        t_char = (self.opt.SNchar/cp*((self.opt.Npix*(self.opt.sigma_r**2/
        self.opt.t_exp + self.opt.dr)*(1.+1./self.opt.Ndark) + Z) + cp + cs))**2
        t_char = t_char*u.s
    ...
    def next_target(self, s_ind, targlist, revisit_list, extended_list, DRM):
        ...
        if self.opt.haveOcculter:
            ...
            ao = self.obs.thrust/self.obs.sc_mass
            targetSep = self.obs.occulterSep
            # find position vector of previous target star
            r_old = self.obs.starprop(self.timekeeping.currenttimeAbs, self.targlist, s_ind)
            # find position vector of new target star            
            r_new = self.obs.starprop(self.timekeeping.currenttimeAbs, self.targlist, new_s_ind)
            # find unit vectors
            ...
            slew_time = np.sqrt(slew_dist/np.abs(ao)/(self.obs.defburnPortion/2. - self.obs.defburnPortion**2/4.))
            mass_used = slew_time*self.obs.defburnPortion*self.obs.flowRate
            ...
            DRM['slew_dV'] = (ao*slew_time*self.obs.defburnPortion).to(u.m/u.s).value
