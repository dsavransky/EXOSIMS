...
class TestPlanetPopulation(unittest.TestCase):
    ...
    def test_honor_erange(self):
        ...
        for mod in self.allmods:
            if mod.__name__ not in exclude_setrange:
                ...
                self.assertTrue(obj.erange[0] == erangein.min(), 'e low bound set failed for %s' % mod.__name__)
                self.assertTrue(obj.erange[1] == erangein.max(), 'e high bound set failed for %s' % mod.__name__)
    ...
    def test_honor_prange(self):
        ...
        for mod in self.allmods:
            if mod.__name__ not in exclude_setrange:
                ...
                self.assertTrue(obj.prange[0] == prangein.min(),'p low bound set failed for %s' % mod.__name__)
                self.assertTrue(obj.prange[1] == prangein.max(),'p high bound set failed for %s' % mod.__name__)
    ...
    def test_honor_Rprange(self):
        ...
        for mod in self.allmods:
            if mod.__name__ not in exclude_setrange:
                ...
                self.assertTrue(obj.Rprange[0].value == Rprangein.min(),'Rp low bound set failed for %s'%mod.__name__)
                self.assertTrue(obj.Rprange[1].value == Rprangein.max(),'Rp high bound set failed for %s'%mod.__name__)
    ...
    def test_honor_Mprange(self):
        ...
        for mod in self.allmods:
            ...
            if mod.__name__ not in exclude_setrange:
                self.assertTrue(obj.Mprange[0].value == Mprangein.min(),'Mp low bound set failed for %s'%mod.__name__)
                self.assertTrue(obj.Mprange[1].value == Mprangein.max(),'Mp high bound set failed for %s'%mod.__name__)
    ...
    def test_honor_wrange(self):
        ...
        for mod in self.allmods:
            if (mod.__name__ not in exclude_setrange) and (mod.__name__ not in exclude_checkrange) and ('gen_angles' in mod.__dict__):
                ...
                self.assertTrue(obj.wrange[0].value == wrangein.min(),'w low bound set failed for %s'%mod.__name__)
                self.assertTrue(obj.wrange[1].value == wrangein.max(),'w high bound set failed for %s'%mod.__name__)
    ...
    def test_honor_Irange(self):
        ...
        for mod in self.allmods:
            if (mod.__name__ not in exclude_setrange) and (mod.__name__ not in exclude_checkrange) and ('gen_angles' in mod.__dict__):
                ...
                self.assertTrue(obj.Irange[0].value == Irangein.min(),'I low bound set failed for %s'%mod.__name__)
                self.assertTrue(obj.Irange[1].value == Irangein.max(),'I high bound set failed for %s'%mod.__name__)
    ...
    def test_honor_Orange(self):
        ...
        for mod in self.allmods:
            if (mod.__name__ not in exclude_setrange) and (mod.__name__ not in exclude_checkrange) and ('gen_angles' in mod.__dict__):
                ...
                self.assertTrue(obj.Orange[0].value == Orangein.min(),'O low bound set failed for %s'%mod.__name__)
                self.assertTrue(obj.Orange[1].value == Orangein.max(),'O high bound set failed for %s'%mod.__name__)
    ...
    def test_gen_angles(self):
        ...
        for mod in self.allmods:
            if 'gen_angles' in mod.__dict__:
                ...
                self.assertEqual(len(O), num, 'Incorrect number of samples generated for %s' % mod.__name__)
                ...
                self.assertEqual(len(w), num, 'Incorrect number of samples generated for %s' % mod.__name__)
