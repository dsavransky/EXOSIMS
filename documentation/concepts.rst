...
:numref:`fig:templates_G0V` shows three G0V spectra pulled from each of the three atlases, along with ``synphot``'s default Vega spectrum and Johnson-V filter profile. Re-normalizing to zero vegamag has minimal effect on both spectra, as shown in :numref:`fig:pickles_bpgs`, but does highlight the differences between their normalizations and the Vega spectrum used preferentially by ``synphot``. The values in the legend represent the total integrated flux of each spectrum in the V-band filter.

.. _fig:templates_diff_G0V:
.. figure:: templates_diff_G0V.png
   :width: 100.0%
   :alt: Difference between Vega re-normalized G0V spectra from MUSCLES, Pickles, and BPGS. 
    
   Difference between re-normalized G0V spectra from MUSCLES, Pickles, and BPGS.

:numref:`fig:templates_diff_G0V` shows the differences between the three spectral templates provided. The three templates typically agree to within :math:`\sim 100 \textrm{ photons cm}^{-2}\textrm{ s}^{-1}\, \mathring{A}^{-1}`. :numref:`fig:pickles_bpgs_diffs` shows the differences between the original Pickles and BPGS spectra and their normalizations, as well as the difference between the two normalized spectra.
...
.. figure:: gauss_box_bandpasses.png
   ...
   Equivalent bandwidth Gaussian (blue) and box (red) filters for 10% bands centered at 500 nm and 1.5 :math:`\mu\mathrm{m}` overlaid on the G0V spetrum from :numref:`fig:templates_G0V`. The bandpasses have amplitudes of 1 and are arbitrarily scaled for visualization purposes. 
