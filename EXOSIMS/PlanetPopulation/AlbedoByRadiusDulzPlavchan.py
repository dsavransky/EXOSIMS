...
class AlbedoByRadiusDulzPlavchan(DulzPlavchan):
    ...
    def __init__(self,  starMass=1., occDataPath=None, esigma=0.175/np.sqrt(np.pi/2.),\
            ps=[0.2,0.5], Rb=[1.4], **specs):
        
        specs['prange'] = [np.min(ps), np.max(ps)]
        DulzPlavchan.__init__(self, starMass=starMass, occDataPath=occDataPath, esigma=esigma, **specs)
