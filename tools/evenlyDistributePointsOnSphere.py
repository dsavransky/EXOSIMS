...
def pt_pt_distances(xyzpoints):
    ...
    closest_point_inds = (
        list()
    )  # list of numpy arrays containing closest points to a given ind
    for i in np.arange(len(xyzpoints)):
        xyzpoint = xyzpoints[i]  # extract a single xyz point on sphere
        diff_pts = (
            xyzpoints - xyzpoint
        )  # calculate linear difference between point spacing
        d_diff_pts = np.linalg.norm(
            diff_pts, axis=1
        )  # calculate linear distance between points
        ss_d, ss_ind = secondSmallest(
            d_diff_pts
        )  # we must get the second smallest because the smallest is the point itself
...
def nlcon2(vvv, ind):
    ...
    xxx = vvv[::3][
        ind
    ]  # this decodes the x vars, vvv[0] and every 3rd element after that
    yyy = vvv[1::3][
        ind
    ]  # this decodes the y vars, vvv[1] and every 3rd element after that
    zzz = vvv[2::3][
        ind
    ]  # this decodes the z vars, vvv[2] and every 3rd element after that
    xyzpoint = np.asarray(
        [xxx, yyy, zzz]
    )  # [[xxx[i], yyy[i], zzz[i]]for i in np.arange(len(zzz))])
    return np.linalg.norm(xyzpoint) - 1.0  # I just want the length to be 1
...
def plotAllPoints(x, y, z, f, x0, con):
    ...
    out01k = minimize(
        f, x0, method="SLSQP", constraints=(con), options={"ftol": 1e-4, "maxiter": 100}
    )
    ...
    out4k = minimize(
        ...
        method="SLSQP",
        constraints=(con),
        options={"ftol": 1e-4, "maxiter": 4000},
    )
    out4kx, out4ky, out4kz = splitOut(out4k)
    ax.scatter(out4kx, out4ky, out4kz, color="cyan")
    plt.legend(
        ["Initial", "100 iter.", "1k iter.", "2k iter.", "4k iter."], loc="uplter left"
    )
...
def initialXYZpoints(num_pts=30):
    ...
    v = np.asarray(
        [[x[i], y[i], z[i]] for i in np.arange(len(x))]
    )  # an array of each point on the sphere
...
if __name__ == "__main__":
    ...
    out1k = minimize(
        ...
        constraints=(con),
        options={"ftol": 1e-4, "maxiter": 1000},
    )  # run optimization problem for 1000 iterations
    ...
    out1kx, out1ky, out1kz = splitOut(out1k)
    out1kv = np.asarray(
        [[out1kx[i], out1ky[i], out1kz[i]] for i in np.arange(len(out1kx))]
    )
    ...
    out01kv = np.asarray(
        [[out01kx[i], out01ky[i], out01kz[i]] for i in np.arange(len(out01kx))]
    )
    out1kv = np.asarray(
        [[out1kx[i], out1ky[i], out1kz[i]] for i in np.arange(len(out1kx))]
    )
    out2kv = np.asarray(
        [[out2kx[i], out2ky[i], out2kz[i]] for i in np.arange(len(out2kx))]
    )
    out4kv = np.asarray(
        [[out4kx[i], out4ky[i], out4kz[i]] for i in np.arange(len(out4kx))]
    )
    ...
    meanDistances = np.asarray(
        [np.mean(dist01k), np.mean(dist1k), np.mean(dist2k), np.mean(dist4k)]
    )
