...
class mDOT_targCAT(StarCatalog):
    ...
    This class contains the functions necessary to populate the StarCatalog with a given mDOT target list,
    using SIMBAD and astroquery.
    ...
    def __init__(self, targlist="mDOT_targs_NH.txt", **specs):
        ...
        Args:
            targlist (string):
                Path to list of catalog names. Defaults to North Hemisphere list as detailed in mDOT concept study,
                Appendix 9.1, table A9.3. File should be readable by the pandas.read_csv method and organized such that
                there is a single column titled "Name" under which the names (legible by SIMBAD) are listed.
        ...
        for i in range(len(table['MAIN_ID'])):
            print(str(table['MAIN_ID'][i]))

        StarCatalog.__init__(self, ntargs=len(table), **specs)

        # list of astropy attributes
        self.dist = (1/(1000*table['PLX_VALUE'].data))*u.pc # calculates distance from parallax measurement in mas
        self.parx = self.dist.to('mas', equivalencies=u.parallax()) # parallax
        self.coords = SkyCoord(ra=table['RA_d'].data*u.deg, dec=table['DEC_d'].data*u.deg,
        ...
        self.MV = self.Vmag - 5.*(np.log10(self.dist.to('pc').value) - 1.) # absolute V magnitude
