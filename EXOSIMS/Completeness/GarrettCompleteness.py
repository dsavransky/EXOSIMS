...
class GarrettCompleteness(BrownCompleteness):
    ...
    def mindmag(self, s):
        ...
        lomask = s < self.rmin*np.sin(self.bstar)
        mindmag[lomask] = -2.5*np.log10(self.pmax*(self.Rmax*self.x/self.rmin)**2\
                *self.Phi(np.arcsin(np.clip(s[lomask]/self.rmin,-1,1))))
        upmask = s > self.rmax*np.sin(self.bstar)
        mindmag[upmask] = -2.5*np.log10(self.pmax*(self.Rmax*self.x/self.rmax)**2\
                *self.Phi(np.arcsin(np.clip(s[upmask]/self.rmax,-1,1))))
