...
#        breakpoint()
        ...
        # 8.3257 d period
#        dV,ang,dt = self.generate_dVMap(fTL,26,f_sInds,self.equinox[0])
#        dV,ang,dt = self.generate_dVMap(fTL,43,f_sInds,self.equinox[0])
#        dV,ang,dt = self.generate_dVMap(fTL,41,f_sInds,self.equinox[0])

        # 17ish d period
        # DRO 72->52 in 5.555 d, 52->99 in 8.055, 99->4 in 8.055
#        dV,ang,dt = self.generate_dVMap(fTL,72,f_sInds,self.equinox[0])
#        dV,ang,dt = self.generate_dVMap(fTL,52,f_sInds,self.equinox[0])
#        dV,ang,dt = self.generate_dVMap(fTL,99,f_sInds,self.equinox[0])

        # L2N 0->13 in 2.555 d, 13->9 in 3.055 d, 9->26 in 2.055 d
        # 11.0244 d period
#        dV,ang,dt = self.generate_dVMap(fTL,0,f_sInds,self.equinox[0])
#        dV,ang,dt = self.generate_dVMap(fTL,13,f_sInds,self.equinox[0])
        dV,ang,dt = self.generate_dVMap(fTL,9,f_sInds,self.equinox[0])
...
#                    rp2 = C_R2B @ (rp - r2)
#
#                    rp1 = self.convertPos_to_dim(rp1)
#                    rp2 = self.convertPos_to_dim(rp2)
#
#                    m1 = 5.97219*10**24*u.kg
#                    m2 = 7.349*10**22*u.kg
#
...
#                    return f1, f2
##
#                acc_ss = np.array([])*u.m/u.s**2
#                acc_ts = np.array([])*u.m/u.s**2
#                for ii in np.arange(len(t[0:1])):
#                    timeT = t[ii]
#                    timeTd = self.convertTime_to_dim(timeT)
#                    timeTd = tA + timeTd
#                    L2_d = self.convertPos_to_canonical(self.L2_dist)
#                    oSep = self.convertPos_to_canonical(self.occulterSep)
#
#                    r_halo = self.haloPosition(timeTd)
#                    st = (self.convertPos_to_canonical(r_halo) + L2_d*np.array([1,0,0]))[0]
#                    sp = (uA * oSep + st)
#
#                    sv = self.convertVel_to_canonical(self.haloVelocity(timeTd)[0])
#
...
#                    fts = f1_ts + f2_ts
#                    
#                    acc_ss = np.append(acc_ss,np.linalg.norm(fss))
#                    acc_ts = np.append(acc_ts,np.linalg.norm(fts))
#
##                Fm1 = F1SS*14500*u.kg
##                Fm2 = F2SS*14500*u.kg
#
##                Fm1_norm = np.round(np.sum(Fm1)/len(Fm1),3)
##                Fm2_norm = np.round(np.sum(Fm2)/len(Fm2),3)
#
#                a_diff = acc_ss - acc_ts
#                
#                r_rel = np.array(state_ss[0:3]) - np.array(state_ts[0:3])
#                rel_hat = r_rel/np.linalg.norm(r_rel)
#
#                a_lat = a_diff - np.dot(a_diff,rel_hat)*rel_hat

#                print("Fm1 avg: " + str(Fm1_norm))
#                print("Fm2 avg: " + str(Fm2_norm))
#                print("a diff burn 1 " + str(np.round(a_diff[0],3)))
#                print("a diff burn 2 " + str(np.round(a_diff[-1],3)))
...
#                ue = Isp*g
#                md_0 = 6722*u.kg
#                mw_0 = 14500*u.kg #np.exp(60/ue.value)*md_0
#                D_0 = 72*u.m
#                R_0 = D_0/2
#                d_0 = 120E3*u.km
...
#                Ft = 1200*u.mN
#                breakpoint()
#                dvAh_dim = self.convertVel_to_dim(np.linalg.norm(dvAh))
#                dvBh_dim = self.convertVel_to_dim(np.linalg.norm(dvBh))
#
#                dV_lt = np.abs(dvAh_dim - dvBh_dim).to('m/s')
#                mp = mw_0*(1 - np.exp(-dV_lt/ue))
#
#                breakpoint()
...
###                    plt.title("Slew Trajectory from Target " + str(nA) + " to " + str(N[x]))
##                    ax.set_xlabel('X [DU]')
##                    ax.set_ylabel('Y [DU]')
##                    ax.set_zlabel('Z [DU]')
##
...
#
#
#                    pos_int = self.convertPos_to_dim(pos_int).to('km')
##                    pos = self.convertPos_to_dim(pos).to('km')
#                    ax = fig.add_subplot(311)
#                    ax.plot(pos_int[:,0],pos_int[:,1],label='integration')
##                    ax.plot(pos[:,0],pos[:,1],label='integration')
#                    ax.set_xlabel('X [km]')
#                    ax.set_ylabel('Y [km]')
#
#                    ax = fig.add_subplot(312)
#                    ax.plot(pos_int[:,2],pos_int[:,1],label='integration')
##                    ax.plot(pos[:,2],pos[:,1],label='integration')
#                    ax.set_xlabel('Z [km]')
#                    ax.set_ylabel('Y [km]')
#
#                    ax = fig.add_subplot(313)
#                    ax.plot(pos_int[:,0],pos_int[:,2],label='integration')
##                    ax.plot(pos[:,0],pos[:,2],label='integration')
#                    ax.set_xlabel('X [km]')
#                    ax.set_ylabel('Z [km]')
#
#                    plt.subplots_adjust(bottom=.1,top=.9,hspace=.7)
##                    plt.subplots_adjust(left=.15,right=.95,bottom=.1,top=.9,hspace=.7)
#                    plt.show()
#                    breakpoint()
##
#
...
#                    ax1.plot(t,pos[:,0] - pos_int[:,0])
##                    ax.set_xlabel('time [TU]')
#                    ax1.set(xlabel=None)
#                    ax1.xaxis.set_tick_params(labelbottom=False)
#                    ax1.set_ylabel(r'$\Delta$X [DU]')
#
#                    ax2 = fig.add_subplot(312, sharex = ax1)
#                    ax2.plot(t,pos[:,1]-pos_int[:,1])
#                    ax2.set(xlabel=None)
#                    ax2.xaxis.set_tick_params(labelbottom=False)
##                    ax.set_xlabel('time [TU]')
#                    ax2.set_ylabel(r'$\Delta$Y [DU]')
#
#                    ax3 = fig.add_subplot(313, sharex = ax1)
#                    ax3.plot(t,pos[:,2]-pos_int[:,2])
#                    ax3.set_xlabel('time [TU]')
#                    ax3.set_ylabel(r'$\Delta$Z [DU]')
#                    
...
#            breakpoint()
    ...
    def send_it2(self, TL, nA, nB, tA, tB, rA, rB):
        """Solves boundary value problem between starshade star alignments
        ...
        sG = np.array(
            [
                [self.rA2[0], self.rB2[0]],
                [self.rA2[1], self.rB2[1]],
                [self.rA2[2], self.rB2[2]],
                [vA[0], vB[0]],
                [vA[1], vB[1]],
                [vA[2], vB[2]],
            ]
        ...
        sol = solve_bvp(
            self.equationsOfMotion_CRTBP, self.boundary_conditions2, t, sG, tol=1e-10, fun_jac = jacobian_CRTBP2)

        ss = sol.y.T
        t_s = sol.x
        status_s = sol.status

        return ss, t_s, status_s
