...
class Test_waypoint(unittest.TestCase):
  ...
  def test1(self):
      ...
      comps = []
      intTimes = [] * u.d 
      self.assertDictEqual(wp.waypoint(comps,intTimes,365,None,None),{"numStars":0,"Total Completeness":0,"Total intTime":0})

      comps = [1,2,3]
      intTimes = [1,2,3] * u.d 
      self.assertDictEqual(wp.waypoint(comps,intTimes,365,None,None),{"numStars":3,"Total Completeness":6,"Total intTime":6*u.d})

      comps = [3,3,3]
      intTimes = [180,180,180] * u.d 
      self.assertDictEqual(wp.waypoint(comps,intTimes,365,None,None),{"numStars":2,"Total Completeness":6,"Total intTime":360*u.d})
