...
class TestTime(unittest.TestCase):
    ...
    def setUp(self):
        ...
        modtype = getattr(
            EXOSIMS.Prototypes.TimeKeeping.TimeKeeping, '_modtype')
    ...
    def test_init(self):
        ...
        req_atts = ['missionStart', 'missionPortion', 'missionLife', 'missionFinishAbs', 'currentTimeNorm',
        ...
        for mod in self.allmods:
            ...
            for att in req_atts:
                self.assertTrue(
                    hasattr(obj, att), 'Missing attribute {} for {}'.format(att, mod.__name__))
    ...
    def test_allocate_time(self):
        ...
        for mod in self.allmods:
            with RedirectStreams(stdout=self.dev_null):
                ...
                self.assertEqual(type(obj.allocate_time(
                    dt, addExoplanetObsTime)), type(False))
    ...
    def test_advanceToAbsTime(self):
        ...
        for mod in self.allmods:
            with RedirectStreams(stdout=self.dev_null):
                ...
                addExoplanetObsTime = False
                self.assertEqual(type(obj.advanceToAbsTime(
                    arbTime, addExoplanetObsTime)), type(False))
                self.assertEqual(type(obj.currentTimeNorm), type(1*u.d))
                self.assertEqual(type(obj.currentTimeAbs), type(arbTime))
                self.assertEqual(type(obj.exoplanetObsTime), type(1*u.d))
        ...
        for mod in self.allmods:
            for det_mode in det_mode_list:
                with RedirectStreams(stdout=self.dev_null):
                    ...
                    self.assertEqual(
                        type(obj.get_ObsDetectionMaxIntTime(Obs, det_mode)[0]), type(1*u.d))
                    self.assertEqual(
                        type(obj.get_ObsDetectionMaxIntTime(Obs, det_mode)[1]), type(1*u.d))
                    self.assertEqual(
                        type(obj.get_ObsDetectionMaxIntTime(Obs, det_mode)[2]), type(1*u.d))
