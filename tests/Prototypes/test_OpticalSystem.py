...
specs_basic.update(specs_default) # add in the above
...
attr_expect = [
    'obscurFac', 'shapeFac', 'pupilDiam', 'telescopeKeepout',
    'attenuation', 'intCutoff', 'Ndark',
    'scienceInstruments', 'starlightSuppressionSystems',
    'IWA', 'OWA', 'dMagLim']
...
opsys_params = dict(
    ...
    telescopeKeepout = dict(default=45.0, trial=(5.0, 5, 90.0, 90), unit=u.deg, target=0),
    attenuation = dict(default=0.57, trial=(0.1, 0.9), unit=float, target=0),
    intCutoff = dict(default=50, trial=(10, 100), unit=u.d, target=0),
    Npix = dict(default=14.3, trial=(5.0, 5, 20), unit=float, target=0),
    Ndark = dict(default=10, trial=(1, 20), unit=float, target=0),
    ...
    scienceInstruments = dict(default=None,
                              trial=(None, {}, [], [{}],
                                     [{'type':'imager'}],[{'type':'imager'},{'type':'spectrograph'}]),
                              raises=(AssertionError,)*4 + (None,)*2,
                              unit=None, target=0),
    # group SIP: science instrument parameters
    lam = dict(default=500, trial=(400, 400.0, 600, 600.0), unit=u.nm, target=1),
    BW = dict(default=0.2, trial=(0.1, 0.9), unit=float, target=1),
    pitch = dict(default=13e-6, trial=(1e-6,1e-2), unit=u.m, target=1),
    ...
    Gem = dict(default=1, trial=(1.5,2), unit=float, target=1),
    ...
    starlightSuppressionSystems = dict(default=None,
                    trial=(None, {}, [], [{}],
                           [{'type':'parasol'}], [{'type':'parasol'},{'type':'umbrella'}]),
                    raises=(AssertionError,)*4 + (None,)*2,
                    unit=None, target=0),
    # group SSP: starlight suppression parameters
    throughput = dict(default=1e-2,
                      trial=(0.02,0.04,
                            "i_quad100.fits","i_quad100t.fits",
                            # errors follow
    ...
    PSF = dict(default=np.ones((3,3)),
               trial=(np.random.rand(3,3), np.random.rand(5,5),
                      ...
                      unit=float, target=2),
    samp = dict(default=10, trial=(), unit=u.arcsec, target=2),
    ohTime = dict(default=1, trial=(), unit=u.d, target=2),
    imagTimeMult = dict(default=1, trial=(), unit=float, target=2),
    charTimeMult = dict(default=1, trial=(), unit=float, target=2),
    # group FP: fundamental IWA, OWA, dMagLim
    ...
    dMagLim = dict(default=None, trial=(18.0, 22.0), unit=float, target=0),
    )
...
class TestOpticalSystemMethods(unittest.TestCase):
    ...
    def setUp(self):
        # print '[setup] ',
        # do not instantiate it -- we need to alter the specs
    ...
    def test_init(self):
        ...
        are in place.
        """
        for specs in [specs_default, specs_basic, specs_simple, specs_multi]:
    ...
    def test_init_occulter(self):
        ...
        Method: If a starlight suppression system has a certain type, the
        attribute OpticalSystem.haveOcculter is set.
        We instantiate OpticalSystem objects and verify that this is done.
        """
        for specs in [specs_default, specs_basic, specs_simple, specs_multi]:
            # the input dict is modified in-place -- so copy it
            for choice in ('external', 'hybrid', 'generic', 'nonsense'):
                our_specs = deepcopy(specs)
                for spec in our_specs['starlightSuppressionSystems']:
                    spec['type'] = choice
                optsys = self.fixture(**deepcopy(our_specs))
                if choice in ('external', 'hybrid'):
                    self.assertTrue(optsys.haveOcculter,
                                    'Expect "%s" suppression system to set haveOcculter' % choice)
                else:
                    self.assertFalse(optsys.haveOcculter,
                                    'Expect "%s" suppression system to clear haveOcculter' % choice)
    ...
    def test_init_iwa_owa_contrast(self):
        ...
        for specs in [specs_default]:
            ...
            # choose attributes
            for (IWA,OWA) in 2*np.random.random((100, 2)):
                # this case is not of interest here, we check it elsewhere
                if IWA >= OWA: continue
                for syst in our_specs['starlightSuppressionSystems']:
                    syst['contrast'] = filename
                    syst['IWA'] = IWA
                    syst['OWA'] = OWA
                if IWA <= 1.0:
                    optsys = self.fixture(**deepcopy(our_specs))
                    ...
                    self.assertAlmostEqual(optsys.dMagLim, expected_dmaglim, delta=1e-3)
                else:
    ...
    def test_init_iwa_owa_throughput(self):
        ...
        lookup table (from 0 to 1).  We verify the half-throughput values, which
        bound the WA, are found correctly.
        """
        filename = os.path.join(resource_path(), 'OpticalSystem', 'i_quad100.fits')
        # These two points are the half-throughput points.  That is, they find the
        # x where throughput is half its maximum (that being 1, at x=0.5) --
        #     tp(x) = 0.25 + 3*x*(1-x) = 0.5
        # Here, x is playing the role of WA.  Solving the quadratic yields:
        IWA_tp = 0.0917517
        OWA_tp = 0.908248
        # we check that these automatically-found values are correct
        for specs in [specs_default]:
            # the input dict is modified in-place -- so copy it
            our_specs = deepcopy(specs_default)
            for syst in our_specs['starlightSuppressionSystems']:
                syst['throughput'] = filename
            optsys = self.fixture(**deepcopy(our_specs))
            # Check that the range constraint of the throughput lookup table
            # (it covers WA in 0 to 1) does constrain the system OWA and IWA.
            self.assertAlmostEqual(optsys.IWA.value, IWA_tp, delta=1e-4,
                                   msg='throughput lookup table IWA')
            self.assertAlmostEqual(optsys.OWA.value, OWA_tp, delta=1e-4,
                                   msg='throughput lookup table OWA')
        # Next, we check that IWA and OWA interact correctly with the throughput
        # lookup table bounds of 0 and 1.
        for specs in [specs_default]:
            # the input dict is modified in-place -- so copy it
            our_specs = deepcopy(specs_default)
            # choose attributes
    ...
    def make_interpolant(self, param, value, unit):
        ...
        if isinstance(value, (numbers.Number, np.ndarray)):
            ...
            elif param in ('throughput', 'contrast', 'PSF'):
            ...
            elif param in ('throughput', 'contrast'):
    ...
    def compare_interpolants(self, f1, f2, msg=''):
