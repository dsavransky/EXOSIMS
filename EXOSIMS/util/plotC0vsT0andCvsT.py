...
class plotC0vsT0andCvsT(object):
    ...
    def singleRunPostProcessing(self, PPoutpath, folder):
        ...
        if 'SLSQPScheduler' in outspec['modules']['SurveySimulation']:
            ...
            ax2.scatter(initt0[initt0.value > 1e-10].value, int_comp[initt0.value > 1e-10], label=r'$c_{3,i}$,' + '' + r'$\sum c_{3,i}$' + "=%0.2f"%sumint_comp, alpha=0.5, color='red', zorder=2, s=45, marker='s')
        ...
        lines = self.writeDATAtoLines(initt0, numObs0, sumOHTIME, raw_det_time, PPoutpath, folder, date, outspec, sim,\
            tmpI, maxCI, minCI, tmpI2, middleCI, int_comp, DRM, star_inds, intTimes, actualComp, comps, alias, data, starsWithPlanets)
        self.lines = lines
    ...
    def writeDATAtoLines(self, initt0, numObs0, sumOHTIME, raw_det_time, PPoutpath, folder, date, outspec, sim,\
        ...
        lines.append(', & , '.join(['sInd'] + unittedListOfAtts + ['Observed'] + ['initt0 (d)'] + ['int_comp'] + ['KnownPlanet']))
