        ...
        if self.aData:
            ...
            for i in xrange(len(Mp_sums)):
                if i == 0:
                    ...
                    U2 = np.random.random(len(inds))
                    Mp[inds] = self.Ms[i]*np.exp(U2*np.log(self.Ms[i+1]/self.Ms[i]))
            ...
            for i in xrange(len(self.Ms) - 1):
                ...
                for j in xrange(len(a_sums)):
                    if j == 0:
                        ...
                        atmp[inds2] = self.smas[0]*np.exp(U2*np.log(self.smas[1]/self.smas[0]))
                    else:
                        inds2 = np.where((U > a_sums[j-1])&(U <= a_sums[j]))[0]
                        U2 = np.random.random(len(inds2))
                        atmp[inds2] = self.smas[j]*np.exp(U2*np.log(self.smas[j+1]/self.smas[j]))
                a[inds] = atmp
            a = a*u.AU
        # Period--planetary radius data given
            ...
            for i in xrange(len(R_sums)):
                if i == 0:
                    ...
                    U2 = np.random.random(len(inds))
                    R_samp[inds] = self.Rs[0]*np.exp(U2*np.log(self.Rs[1]/self.Rs[0]))
                else:
                    inds = np.where((U > R_sums[i-1])&(U <= R_sums[i]))[0]
                    U2 = np.random.random(len(inds))
                    R_samp[inds] = self.Rs[i]*np.exp(U2*np.log(self.Rs[i+1]/self.Rs[i]))
            ...
            for i in xrange(len(self.Rs)-1):
                ...
                for j in xrange(len(P_sums)):
                    if j == 0:
                        ...
                        U2 = np.random.random(len(inds2))
                        Ptmp[inds2] = self.Ps[0]*np.exp(U2*np.log(self.Ps[1]/self.Ps[0]))
                    else:
                        inds2 = np.where((U > P_sums[j-1])&(U <= P_sums[j]))[0]
                        U2 = np.random.random(len(inds2))
                        Ptmp[inds2] = self.Ps[j]*np.exp(U2*np.log(self.Ps[j+1]/self.Ps[j]))
                P_samp[inds] = Ptmp
            # convert period samples to semi-major axis
            a = ((const.G*self.starMass*(P_samp*u.day)**2/4./np.pi**2)**(1./3.)).to('AU')
