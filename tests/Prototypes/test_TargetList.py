...
#scriptfile = os.path.join(EXOSIMS.__path__[0],'Scripts','template_prototype_testing.json') 
...
class Test_TargetList_prototype(unittest.TestCase):
    ...
    def test_dmag_filter(self):
        ...
        self.opticalsystem.dMagLim = np.inf
        self.targetlist.max_dmag_filter()
        self.assertEqual( self.targetlist.nStars , n0)
        #Test limiting case of dMagLim = 0
        self.opticalsystem.dMagLim = 0.0
        with self.assertRaises(IndexError):
            self.targetlist.max_dmag_filter()
    ...
    def test1_dmag_filter(self):
        #Test case of IWA = 0.5 arcsec and dMagLim = 21
        self.opticalsystem.IWA = 0.5 *u.arcsec
        self.opticalsystem.dMagLim = 21
        self.targetlist.max_dmag_filter()
        #assert self.targetlist.nStars == 390 #not very useful
        n1 = self.targetlist.nStars
        #Test limiting case that distance to a star is infinite
        #self.targetlist.dist[10] = np.inf
        #self.targetlist.dist[11] = np.inf
        #self.targetlist.max_dmag_filter()
        #assert self.targetlist.nStars == n1 - 2
        #Test the case when planets are far from star
        # self.planetpop.rrange = np.array([np.inf,np.inf])*u.AU
    ...
    @unittest.skip("This test needs to be revised: the cutoff filter is handled differently now")
    def test_int_cutoff_filter(self):
        ...
        self.targetlist.maxintTime = np.array([0]*int(n0)) * u.day
        self.targetlist.int_cutoff_filter()
        self.assertEqual( self.targetlist.nStars , n0)
        #Test limiting case of infinite max integration time
        self.targetlist.maxintTime = np.array([np.inf]*int(n0)) * u.day
        with self.assertRaises(IndexError):
            self.targetlist.int_cutoff_filter()
        #self.assertEqual( self.targetlist.nStars , 0)

    @unittest.skip("This test needs to be revisited")
    def test_completeness_filter(self):
        n0 = self.targetlist.nStars
        # FIXME: unclear that this is doing what it seems to
        #Test limiting case of minComp = 0
        self.targetlist.completeness_filter()
        self.assertEqual( self.targetlist.nStars , n0)
        #Test default of minComp = 0.1
        self.targetlist.completeness_filter()
        self.assertEqual( self.targetlist.nStars , n0)
    ...
    def test_stellar_mass(self):
        ...
        np.testing.assert_allclose(self.targetlist.MsEst[0], 1.05865, rtol=1e-5, atol=0)
        #Relative tolerance is 0.07 according to Savranski 2010
        np.testing.assert_allclose(self.targetlist.MsTrue[0], 1.05865, rtol=0.07, atol=0)
