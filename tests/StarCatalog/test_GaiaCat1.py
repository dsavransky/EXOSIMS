...
class TestGaiaCat1(unittest.TestCase):
    ...
    def test_init(self): 
        ...
        np.testing.assert_allclose(expected['astrometric_matched_observations'],
            gaia.astrometric_matched_observations)   
        np.testing.assert_allclose(expected['visibility_periods_used'],
        ...
        exp_coords = SkyCoord(ra=expected['ra']*u.deg,
            dec=expected['dec']*u.deg, distance=exp_dist)
        ...
        for i in range(len(exp_coords)):
            exp_coords_array.append([exp_coords[i].ra.degree,
            exp_coords[i].dec.degree, exp_coords[i].distance.pc])
            gaia_coords_array.append([gaia.coords[i].ra.degree,
            gaia.coords[i].dec.degree,gaia.coords[i].distance.pc])
        ...
        expected_vmag = eGmag - (-0.01760 - 0.006860*(eBPmag - eRPmag)\
                - 0.1732*(eBPmag - eRPmag)**2)
        expected_rmag = eGmag - (-0.003226 + 0.3833*(eBPmag - eRPmag)\
                - 0.1345*(eBPmag - eRPmag)**2)
        expected_Imag = eGmag - (0.02085 + 0.7419*(eBPmag - eRPmag)\
                - 0.09631*(eBPmag - eRPmag)**2)
