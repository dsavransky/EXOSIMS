...
class plotCompletenessJointPDFs(object):
    ...
    def plotJointPDFwithCompRectDetBoxes(self, sim, PPoutpath, folder, IWA, OWA, dMagLim,s0,dMag0,s1,dMag1,sigma_s,sigma_dmag):
        """
        Args:
            sim
            PPoutpath
        ...
        #minf = 0
        #levelList = [10**x for x in np.linspace(start=minf,stop=maxf,num=maxf-minf+1, endpoint=True)]
        levelList = np.logspace(start=-5.,stop=0.,num=200)
        ...
        #Add Comp Rectangle
        smax = 10.*u.pc.to('AU')*OWA.to('rad').value #OWA in AU at 10 parsecs
        smin = 10.*u.pc.to('AU')*IWA.to('rad').value #IWA in AU at 10 parsecs
        plt.plot([smin,smin,smax,smax],[0.,dMagLim,dMagLim,0.],color='black',linewidth=2)
        ...
        #Add Det0
        plt.plot([s0-sigma_s,s0-sigma_s,s0+sigma_s,s0+sigma_s,s0-sigma_s],[dMag0-sigma_dmag,dMag0+sigma_dmag,dMag0+sigma_dmag,dMag0-sigma_dmag,dMag0-sigma_dmag],color='black',linewidth=2)
        fname = 'completenessJoinfPDFwithCompRectdet0_' + folder.split('/')[-1] + '_' + date
        plt.savefig(os.path.join(PPoutpath, fname + '.png'), format='png', dpi=500)
        plt.savefig(os.path.join(PPoutpath, fname + '.svg'))
        plt.savefig(os.path.join(PPoutpath, fname + '.eps'), format='eps', dpi=500)
        plt.savefig(os.path.join(PPoutpath, fname + '.pdf'), format='pdf', dpi=500)
        ...
        #Add Det1
        plt.plot([s1-sigma_s,s1-sigma_s,s1+sigma_s,s1+sigma_s,s1-sigma_s],[dMag1-sigma_dmag,dMag1+sigma_dmag,dMag1+sigma_dmag,dMag1-sigma_dmag,dMag1-sigma_dmag],color='black',linewidth=2)
        fname = 'completenessJoinfPDFwithCompRectdet0det1_' + folder.split('/')[-1] + '_' + date
        plt.savefig(os.path.join(PPoutpath, fname + '.png'), format='png', dpi=500)
        plt.savefig(os.path.join(PPoutpath, fname + '.svg'))
        plt.savefig(os.path.join(PPoutpath, fname + '.eps'), format='eps', dpi=500)
        plt.savefig(os.path.join(PPoutpath, fname + '.pdf'), format='pdf', dpi=500)
