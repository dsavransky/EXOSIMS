...
def plot_obs(SS, systemParams, mode, sInd, pInds, SNR, detected):
    ...
    H_scaled = H/10000
    ax.contourf(H_scaled, levels=levels, cmap=my_cmap, origin='lower',
                extent=extent, norm=mpl.colors.LogNorm())
    ...
    ax.axhline(y=TL.saturation_dMag[sInd]-2.5*np.log10(L), color=my_cmap(0),
